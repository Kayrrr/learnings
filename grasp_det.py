...
def run_training():
    ...
    learning_rate=tf.train.exponential_decay(0.0005,global_step,decay_steps=100,decay_rate=0.999,staircase=False)
    train_op = tf.train.AdamOptimizer(learning_rate=learning_rate,epsilon=0.1).minimize(loss)
    ...
    if FLAGS.train_or_validation == 'train':
        saver.restore(sess, "./models/imagenet/m2/m2.ckpt")
    else:
